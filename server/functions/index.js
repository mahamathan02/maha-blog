...
//create blog
//post-title,short-discrition ,content,cover-images

exports.createBlog = functions.https.onRequest(async (req, res) => {
  //enable the cors orgin
  cors(req, res, async () => {
    const id = uuidv4();
  ...
  cors(req, res, async () => {
    try {
      ...
      if (snapShot.exists) {
        ...
        return res.status(200).json("Blog update successfilly");
      } else {
        return res.status(200).json({ id, msg: "Node Data Found" });
      }
    } catch (error) {
...
exports.deleteBlogId = functions.https.onRequest(async (req, res) => {
  cors(req, res, async () => {
    try {
      const id = req.query.id;
      const timestamp = FieldValue.serverTimestamp();
